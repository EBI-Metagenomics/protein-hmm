...
% The notation $\viterbi(q_t)$ under the HMM context, and for a given sequence $\arr{z}$, conveys
% three crucial information: (i) we are at step $t$ of the HMM process; (ii) the score is
% about some state $q_t \in \set{Q}$; (iii) regarding (iii) the prefix $z_{1..t}$. The same notation under
% the IMM context loses the information (iii): the index $t$ no longer unambigously defines a prefix.
...
for $t>1$; and
\begin{equation*}
  \viterbi_{i}(q_1,f_1) = p(S_1=z_{1..i} \gv Q_1=q_1) p(Q_1=q_1).
\end{equation*}
