...
Let us assume that we already have established the error-free codon distribution $p(X_t=x_1x_2x_3
\gv Q_t=q_t)$ from the amino acid distribution of state $q_t$. This section will formally define the
error-prone codon distribution $p(Z_t=z_1z_2\dots \gv Q_t=q_t)$ from distribution $p(X_t=x_1x_2x_3
\gv Q_t = q_t)$ and parameter $\eps$ based on the outlined method.

We will omit the index $t$ and the conditional part of the probability for simplicity. The next
sections will also use an underscore $\s$ to denote a summation over the corresponding sequence
positions. For example, $p(X=x_1\s\s)=\sum_{x_2,x_3}p(X=x_1x_2x_3)$.

\subsubsection{Sequences of length 1}

\begin{equation*}
  p(Z=z_1) = \eps^2(1-\eps)^2(p(X=z_1\s\s) + p(X=\s z_1\s) + p(X=\s\s z_1)) / 3
\end{equation*}
...
\begin{equation*}
  \begin{split}
    p(Z=z_1z_2z_3) &= (1-\eps)^4 p(X=z_1z_2z_3)\\
...
\begin{equation*}
  \begin{split}
    p(Z=z_1z_2z_3z_4) &= \eps(1-\eps)^3 (p(X=z_2z_3z_4)p(z_1)+p(X=z_1z_3z_4)p(z_2)\\
